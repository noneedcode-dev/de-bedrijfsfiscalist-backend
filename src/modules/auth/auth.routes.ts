...
/**
 * @swagger
 * /api/auth/bubble-reset/register:
 *   post:
 *     summary: Register Bubble-generated password reset token
 *     description: Store a password reset token generated by Bubble. This is the first step in the mutual verification flow where Bubble sends the token before the user changes their password.
 *     tags:
 *       - Authentication
 *     security: []
 *     requestBody:
 ...
 *               reset_token:
 *                 type: string
 *                 description: Password reset token generated by Bubble
 *                 example: "bubble-generated-token-12345"
 *     responses:
 *       200:
...
authRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Check if same email + reset_token already exists and is unused
    const { data: existingToken } = await supabase
      .from('bubble_password_reset_tokens')
      .select('*')
    ...
    // If token already exists and is valid, return idempotently
    if (existingToken) {
      logger.info('Bubble reset token already registered (idempotent)', {
        email,
    ...
    if (insertError) {
      logger.error('Failed to register Bubble reset token', {
        error: insertError,
        email,
      });
      throw AppError.fromCode(ErrorCodes.INTERNAL_ERROR, 500);
    ...
    logger.info('Bubble reset token registered successfully', {
      email,
      expires_at: expiresAt.toISOString(),
    });
...
/**
 * @swagger
 * /api/auth/bubble-reset/confirm:
 *   post:
 *     summary: Confirm password reset with Bubble token
 *     description: Validate the Bubble-generated token and update the Supabase password. This is the second step after Bubble successfully resets the user's Bubble password. Token is consumed only after successful password update.
 *     tags:
 *       - Authentication
 *     security: []
 *     requestBody:
 ...
 *               reset_token:
 *                 type: string
 *                 description: Password reset token from Bubble
 *                 example: "bubble-generated-token-12345"
 *               new_password:
 *                 type: string
 *                 format: password
...
authRouter.post(
  ...
  handleValidationErrors,
  asyncHandler(async (req: Request, res: Response) => {
    // Step 1: Validate new_password strength (already done by middleware above)
    const { email, reset_token, new_password } = req.body;
    const supabase = createSupabaseAdminClient();

    // Step 2: Lookup bubble_password_reset_tokens
    const { data: tokenRecord, error: tokenError } = await supabase
      .from('bubble_password_reset_tokens')
      .select('*')
      .eq('email', email.toLowerCase())
    ...
    // Step 3: If not found → return 400 invalid_or_expired_token
    if (tokenError || !tokenRecord) {
      logger.warn('Invalid or expired Bubble reset token attempt', {
        email,
        error: tokenError?.message,
      });
      throw AppError.fromCode(
    ...
    // Step 4: Find Supabase user by email using admin client
    const { data: authUsers, error: listError } =
      await supabase.auth.admin.listUsers();

    if (listError) {
      logger.error('Failed to list users for Bubble password reset', {
        error: listError,
      });
      throw AppError.fromCode(ErrorCodes.INTERNAL_ERROR, 500);
    }
    ...
    if (!user) {
      logger.warn('User not found for Bubble password reset', {
        email,
      });
      throw AppError.fromCode(ErrorCodes.PASSWORD_RESET_USER_NOT_FOUND, 404);
    }

    // Step 5: Update Supabase password using admin
    // IMPORTANT: If this fails, token remains unused (not marked as used)
    const { error: updateError } = await supabase.auth.admin.updateUserById(
      user.id,
      {
        ...
        user_metadata: {
          ...user.user_metadata,
          bubble_password_reset_at: new Date().toISOString(),
        },
      }
    );
    ...
    if (markUsedError) {
      logger.error('Failed to mark Bubble reset token as used', {
        error: markUsedError,
        tokenId: tokenRecord.id,
      });
      // Don't fail the request - password was already updated successfully
    ...
    logger.info('Bubble password reset successful', {
      userId: user.id,
      email,
    });
