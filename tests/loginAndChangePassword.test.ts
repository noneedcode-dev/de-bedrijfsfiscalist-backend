...
// Hoist dependencies for vi.mock() factory - must be self-contained
const { mockUserId, mockAccessToken } = vi.hoisted(() => {
  // Import jsonwebtoken inline to avoid hoisting issues
  const jsonwebtoken = require('jsonwebtoken');
  const jwtSecret = process.env.SUPABASE_JWT_SECRET || 'test-jwt-secret';
  const mockUserId = '123e4567-e89b-12d3-a456-426614174000';
  const mockAccessToken = jsonwebtoken.sign(
    { sub: mockUserId, email: 'user@example.com', role: 'client' },
    jwtSecret,
    { expiresIn: '1h' }
  );
  return { mockUserId, mockAccessToken };
});

// Hoist all mock functions to avoid initialization issues
const { createMockChain, mockSignInWithPassword, mockUpdateUserById, mockGetUser } = vi.hoisted(() => {
  ...
  const mockUpdateUserById = vi.fn(() => ({ data: { user: null }, error: null }));

  const mockGetUser = vi.fn(() => ({
    data: { user: { id: mockUserId, email: 'user@example.com', role: 'client', client_id: '123e4567-e89b-12d3-a456-426614174000' } },
    error: null,
  }));

  return { createMockChain, mockSignInWithPassword, mockUpdateUserById, mockGetUser };
});
...
vi.mock('../src/lib/supabaseClient', () => {
  ...
  return {
    createSupabaseAdminClient: vi.fn(() => createMockSupabase()),
    createSupabaseUserClient: vi.fn(() => createMockSupabase()),
  };
});
...
describe('Login and Change Password Flow', () => {
  ...
  describe('Integration: Login then Change Password', () => {
    it('should allow password change after successful login', async () => {
      ...
      // Verify we got a valid JWT token
      expect(typeof accessToken).toBe('string');
      expect(accessToken.length).toBeGreaterThan(0);

      // Step 2: Change password using a fresh token (mock limitation workaround)
      // The mock returns the same token for all requests, so we use a fresh one
